...
.PHONY: all setup test cpu gpu \
        ...
        beth audit cpu-all-1epoch ip-cpu ip-transition-cpu ip-study-cpu test-unit check-study \
...
ip-transition-cpu: setup
	$(VENV_PYTHON) scripts/run_ip_transition_experiment.py $(IP_TRANSITION_CPU_ARGS)
...
help:
	...
	@echo "  make cpu               Run the local CPU benchmark path"
	...
	@echo ""
	@echo "GPU Workflows:"
	@echo "  make gpu               Run the accelerator benchmark path"
	...
	@echo "  make hardware          Alias for make int8"
	@echo ""
	@echo "Cleanup:"
